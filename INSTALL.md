# Install Job Apply Autopilot V5.11

V5.11 upgrades the existing campaign in place. Do **not** delete `<workspace>\.job-apply-autopilot`.

From the chosen campaign workspace:

```powershell
$zip  = ".\job-apply-autopilot-v5.11.zip"
$temp = Join-Path $env:TEMP "job-apply-autopilot-v5.11"
$dst  = "$HOME\.config\opencode\skills\job-apply-autopilot"

Remove-Item $temp -Recurse -Force -ErrorAction SilentlyContinue
Expand-Archive -LiteralPath $zip -DestinationPath $temp -Force
$src = Join-Path $temp "job-apply-autopilot-v5.11"

...
The user explicitly requested this persistent campaign. If goal_status and goal_set are available, inspect goal state once and create the continuous coordinator goal described by the skill when none is active. Do not mark it complete for a submission count, a temporarily empty queue, a blocked route, or this slice ending; only the user's /goal pause or /goal stop intentionally halts it. Goal continuation does not authorize duplicate external side effects, and workers never create goals.

This is autonomous supervisor slice $session. Act immediately from session-state.ps1. Optimize for NET-NEW unique company/title submissions per hour, not raw ledger rows or single-job latency. Group actions by dispatch and emit every eligible Task call together in one assistant turn. Assessor, unified research, resume, external ATS, and email workers are uncapped by skill policy and run up to runtime capacity; only LinkedIn Easy Apply is serial at 1 under its governor. Keep web-heavy research out of a fast-worker wave when the harness waits for all calls. A worker prompt is exactly two lines: Work item directory: <absolute-path>, then Action: <action>; never paste policy or job summaries. The local assessor may request one decision-changing research kind; job-autopilot-research consumes existing reports first, performs only bounded missing lookup, and commits final pass/fail without another assessor call. Keep the minimum 8-job pipeline buffer filled after ready work; discovery must capture multiple full JDs rather than stopping after one. Do not re-apply to a repost or regional duplicate. Capture source_pending jobs as part of intake without blocking ready assessments. Close disposable BrowserOS tabs promptly, except unresolved CAPTCHA tabs: preserve them, trigger the installed solver once when an ordinary challenge control is available, and wait up to 120 seconds as documented in references/captcha-recovery.md. If BrowserOS becomes unavailable, persist local state and end this slice; do not call it campaign/runtime or goal completion because the supervisor will retry after BrowserOS health returns. Keep taking useful application or discovery actions for the full slice. Never ask routine questions; do not manually solve challenge puzzles, synthesize tokens, or bypass MFA/security controls.

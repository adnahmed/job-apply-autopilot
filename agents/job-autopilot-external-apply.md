...
Security: first spam/automation/429/security/CAPTCHA/MFA signal means zero Submit retries. Record it through `domain-circuit-breaker.ps1 -Action Record`; do not hand-append JSONL. Ordinary form validation gets one correction.
...
Return one line: `submitted <domain> <confirmation>`, `handoff-email <address>`, or `blocked <domain> <reason>`.

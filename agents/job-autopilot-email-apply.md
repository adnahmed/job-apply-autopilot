...
1. Reserve with `-Channel email -Target <recipient> -Subject <subject>` before Compose. Stop on `already-submitted`; write `skipped-duplicate` through the outcome writer on `semantic-already-submitted`; defer on `semantic-reservation-exists`.
